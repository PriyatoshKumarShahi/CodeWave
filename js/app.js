document.addEventListener('DOMContentLoaded', function () {
  ...
  editors.html.on('keydown', (cm, event) => {
      ...
      if (event.key === 'Enter' && tagsToAutoClose.includes(lastWord)) {
          ...
          cm.setCursor({ line: cursor.line, ch: start.ch + tagName.length + 2 }); // Cursor between the opening and closing tags
  ...
  editors.html.on('change', (instance, changeObj) => {
      if (changeObj.origin === '+input' && changeObj.text.length === 1) {
          ...
          hideHint(); // Hide hint if not typing a tag
      }
  });

  // Load, save, copy, clear, and update content functions
  loadSavedCode();
